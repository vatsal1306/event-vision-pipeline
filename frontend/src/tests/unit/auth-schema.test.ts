...
describe('Auth Schemas', () => {
  describe('loginSchema', () => {
    it('should validate email or phone with password', () => {
    ...
    it('should fail on empty identifier', () => {
      const result = loginSchema.safeParse({ email_or_phone: '', password: 'Password1!' });
      expect(result.success).toBe(false);
    });

    it('should fail on empty password', () => {
      const result = loginSchema.safeParse({ email_or_phone: 'test@example.com', password: '' });
      expect(result.success).toBe(false);
      if (!result.success) {
