...
class Flatten(Module):  # added to fix the reshape layer issue after conversion.
    def forward(self, x):
        return torch.flatten(x, start_dim=1)  # Ensures ONNX conversion keeps correct shape
...
class GDCONNX(Module):
    def __init__(self, embedding_size):
        super(GDCONNX, self).__init__()
        self.layers = nn.Sequential(
            LinearBlock(512, 64, groups=1, kernel=(1, 1), stride=(1, 1), padding=(0, 0)),
            Flatten(),
            Linear(64 * 49, embedding_size, bias=True),
            BatchNorm1d(embedding_size))
...
class MobileFaceNet(Module):
    def __init__(self, fp16=False, num_features=512, blocks=(1, 4, 6, 2), scale=2, onnx_arch=False):
        ...
        if onnx_arch:
            self.features = GDCONNX(num_features)
        else:
...
def get_mbf_onnx(fp16, num_features, blocks=(1, 4, 6, 2), scale=2):
    return MobileFaceNet(fp16, num_features, blocks, scale=scale, onnx_arch=True)
...
def get_mbf_large_onnx(fp16, num_features, blocks=(2, 8, 12, 4), scale=4):
    return MobileFaceNet(fp16, num_features, blocks, scale=scale, onnx_arch=True)
...
def get_model(name, **kwargs):
    ...
    if name == "r18":
        ...
        fp16 = kwargs.get("fp16", False)
        num_features = kwargs.get("num_features", 512)
        return get_mbf_onnx(fp16=fp16, num_features=num_features)
    ...
    elif name == "mbf_large_onnx":
        from .mobilefacenet import get_mbf_large
        fp16 = kwargs.get("fp16", False)
        num_features = kwargs.get("num_features", 512)
        return get_mbf_large_onnx(fp16=fp16, num_features=num_features)
