...
- Do **not** run `face_processing` on the production app EC2. That host is CPU-only (`c6a.xlarge`): queues `photo_processing` and `notifications` only, Celery concurrency **2–3**. Face/ML workers run on the on-demand GPU host (INF-009). GPU start/stop tasks stay on `photo_processing`.
